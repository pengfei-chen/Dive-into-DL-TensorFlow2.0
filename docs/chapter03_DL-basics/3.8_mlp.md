...
我们已经介绍了包括线性回归和softmax回归在内的单层神经网络。然而**深度学习主要关注多层模型**。在本节中，我们将以多层感知机（multilayer perceptron，MLP）为例，介绍多层神经网络的概念。
...
上述问题的根源在于全连接层只是对数据做仿射变换（affine transformation），**而多个仿射变换的叠加仍然是一个仿射变换**。解决问题的一个方法是**引入非线性变换**，例如对隐藏变量使用按元素运算的非线性函数进行变换，然后再作为下一个全连接层的输入。这个非线性函数被称为激活函数（activation function）。下面我们介绍几个常用的激活函数。
