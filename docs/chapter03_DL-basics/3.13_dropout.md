...
下面定义的模型将全连接层和激活函数ReLU串起来，并**对每个激活函数的输出使用丢弃法**。我们可以**分别设置各个层的丢弃概率**。通常的建议是**把靠近输入层的丢弃概率设得小一点**。在这个实验中，我们把第一个隐藏层的丢弃概率设为0.2，把第二个隐藏层的丢弃概率设为0.5。我们可以通过参数`is_training`函数来判断运行模式为训练还是测试，并只需在训练模式下使用丢弃法。
